...
No functional changes — the same operations, parameters and API behaviour.

### Fixed

- The codex `node` identifier is now the fully-qualified
  `@quickemailverification/n8n-nodes-quickemailverification.quickEmailVerification`
  rather than the bare `quickEmailVerification`, matching the
  `<package-name>.<nodeName>` format n8n requires.

### Changed

- Codex `categories` are now `Utility` and `Communication`. The previous
  `Marketing` value is not in n8n's supported set and was silently dropped by
  the UI.
